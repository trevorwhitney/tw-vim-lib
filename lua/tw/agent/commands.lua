...
local function resolve_default_agent_buf()
	...
	local inst = agent._get_instance(default, 0)
...
local function handle_list_contexts(claude_module, _args)
	...
	local lines = { "Context directories:" }
	local i = 1
	for source_path, _ in pairs(claude_module.context_directories) do
		table.insert(lines, string.format("  %d. %s", i, source_path))
		i = i + 1
	end
...
local function handle_aiagent_command(args, agent_module)
	...
	if not subcommand then
		...
		vim.notify(
			"Available subcommands: add-context, remove-context, list-contexts, show-log, log-level, clear-scrollback, toggle-follow, fullscreen",
			vim.log.levels.INFO
		)
		return
	end
