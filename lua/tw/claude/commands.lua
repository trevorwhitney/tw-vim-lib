...
local function handle_shell(claude_module, args)
  ...
  if not found then
    ...
    claude_module.shell_job_id = vim.fn.termopen("docker exec -it " .. claude_module.container_name .. " /bin/bash", {
...
local function handle_container_logs(claude_module, args)
  ...
  if not found then
    ...
    claude_module.logs_job_id = vim.fn.termopen(
      "docker exec -it " .. claude_module.container_name .. " /bin/bash -c 'for dir in /home/node/.cache/claude-cli-nodejs/-workspace /home/node/.cache/claude-cli-nodejs; do if [ -d \"$dir\" ]; then find \"$dir\" -name \"*.log\" -type f -exec echo \"=== {} ===\" \\; -exec cat {} \\; -exec echo \"\" \\; 2>/dev/null; fi; done || echo \"No log files found\"'",
      {
        on_exit = function(_, exit_code)
...
local function handle_claude_docker_command(args, claude_module)
  ...
  if not subcommand then
    ...
    vim.notify("Available subcommands: toggle, build, restart, add-context, remove-context, list-contexts, shell, show-log, container-logs, log-level, check-firewall", vim.log.levels.INFO)
    return
  end
  ...
  vim.api.nvim_create_user_command("ClaudeDocker", function(args)
    ...
    complete = function(arg_lead, cmd_line, cursor_pos)
      ...
      if num_args == 0 or (num_args == 1 and not cmd_line:match("%s$")) then
        local subcommands = {
          "toggle", "build", "restart", "add-context", "remove-context",
          "list-contexts", "shell", "show-log", "container-logs",
          "log-level", "check-firewall"
        }
