...
M.check = function()
  ...
  if is_running then
    ...
    if docker.check_firewall_status(container_name) then
      ...
      local test_cmd = "docker exec " .. container_name ..
      ...
      if handle then
        ...
        if result == "" or result == "000" then
          ...
          health.warn("Firewall may not be blocking properly",
      ...
      health.warn("Container firewall not configured",
        "Firewall will be set up on next container start")
    end
  else
  ...
  if vim.env.ANTHROPIC_API_KEY then
    ...
    health.error("ANTHROPIC_API_KEY not set",
  ...
  if vim.env.GH_TOKEN or vim.env.GITHUB_PERSONAL_ACCESS_TOKEN then
    ...
    health.warn("No GitHub token found",
      "Set GH_TOKEN or GITHUB_PERSONAL_ACCESS_TOKEN for GitHub operations")
  end
