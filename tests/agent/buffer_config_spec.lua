...
describe("buffer-config lifecycle", function()
	local buffer_config
	...
	it("does not install terminal-output status tracking", function()
		local buf = vim.api.nvim_create_buf(false, true)
		buffer_config.setup_buffer(buf, {})
		local autocmds = vim.api.nvim_get_autocmds({ group = "ClaudeBuffer_" .. buf })
		for _, autocmd in ipairs(autocmds) do
			assert.is_not.equals("TextChanged", autocmd.event)
			assert.is_not.equals("TextChangedT", autocmd.event)
		end
