...
const escCols = `id, job_id, ts, kind, question, advice, action_kind, action_params_json,
	state, resolution, reason, last_notified_ts`
...
func scanEscalation(row interface{ Scan(...any) error }) (Escalation, error) {
	...
	err := row.Scan(&e.ID, &e.JobID, &e.TS, &e.Kind, &e.Question, &e.Advice,
		&e.ActionKind, &e.ActionParamsJSON, &e.State, &e.Resolution, &e.Reason, &e.LastNotifiedTS)
	return e, err
}
...
func (s *Store) ResolveEscalation(id int64, resolution, reason string) error {
	_, err := s.db.Exec(
		`UPDATE escalations SET state='resolved', resolution=?, reason=?, resolved_ts=? WHERE id=? AND state='open'`,
		resolution, reason, s.now().Unix(), id)
	return err
}
